#!/usr/bin/env python3
"""
Railway Build Trigger File

This file exists solely to trigger Railway builds when no other backend files have changed.
If Railway reports "No changed files matched patterns: backend/**", make a simple edit to this file
(like changing the version number below) and commit the change.

Version: 1.1.2
Last update: Fixed Pinecone dimension mismatch (3072 dimension consistency)
"""

print("Railway build trigger file - This file helps ensure Railway detects changes in the backend directory")

# No actual functionality needed - this is just a build trigger
